...
static CGFloat cachedExpandedHeight = 0;
...
@interface MediaControlsPanelViewController (ORHMusicModule)
...
- (MediaControlsTimeControl *)bcm_timeControl {
	if (self.isBCMMusicModule) {
		...
		if (parentController && [parentController isKindOfClass:NSClassFromString(@"MediaControlsPanelViewController")]) {
			MediaControlsPanelViewController *panelController = (MediaControlsPanelViewController *)parentController;
...
- (void)setSecondaryString:(NSString *)string {
	if (self.isBCMMusicModule && shouldFakeStyle) {
		...
		MediaControlsPanelViewController *sharedController = (MediaControlsPanelViewController *)[self _viewControllerForAncestor];
		if (sharedController && [sharedController isKindOfClass:NSClassFromString(@"MediaControlsPanelViewController")]) {
			if (!SYSTEM_VERSION_GREATER_THAN_OR_EQUAL_TO(@"11.2")) {
...
-(id)moduleSettingsForModuleIdentifier:(NSString *)identifier prototypeSize:(CCUILayoutSize)protoSize {
	if (identifier) {
		if ([identifier isEqualToString:@"com.apple.mediaremote.controlcenter.nowplaying"]) {
			CCUILayoutSize layoutSize;
			if (musicModSizeOption == 1) layoutSize = CCUILayoutSizeMake(4,2);
			if (musicModSizeOption == 2) layoutSize = CCUILayoutSizeMake(3,2);
			if (musicModSizeOption == 3 || musicModSizeOption == compactDisabledOpt) layoutSize = CCUILayoutSizeMake(2,2);
		...
		if ([identifier isEqualToString:@"com.apple.control-center.ConnectivityModule"]) {
			CCUILayoutSize layoutSize;
			if (connectModSizeOption == 1) layoutSize = CCUILayoutSizeMake(4,1);
			if (connectModSizeOption == 2) layoutSize = CCUILayoutSizeMake(3,2);
			if (connectModSizeOption == 3) layoutSize = CCUILayoutSizeMake(2,3);
			if (connectModSizeOption == 4) layoutSize = CCUILayoutSizeMake(2,2);
			return [[NSClassFromString(@"CCUIModuleSettings") alloc] initWithPortraitLayoutSize:layoutSize landscapeLayoutSize:layoutSize];
		}
		if (SYSTEM_VERSION_GREATER_THAN_OR_EQUAL_TO(@"12.0")) {
			if ([identifier isEqualToString:@"com.atwiiks.betterccxi.weathermodule11"]) {
				CCUILayoutSize layoutSize;
				layoutSize = CCUILayoutSizeMake(0,0);
				return [[NSClassFromString(@"CCUIModuleSettings") alloc] initWithPortraitLayoutSize:layoutSize landscapeLayoutSize:layoutSize];
			}
		}
		if ([identifier isEqualToString:@"com.atwiiks.betterccxi.weathermodule"] || [identifier isEqualToString:@"com.atwiiks.betterccxi.weathermodule11"]) {
			CCUILayoutSize layoutSize;
			if (weatherModSizeOption == 1) layoutSize = CCUILayoutSizeMake(4,2);
			if (weatherModSizeOption == 2) layoutSize = CCUILayoutSizeMake(4,1);
			if (weatherModSizeOption == 3) layoutSize = CCUILayoutSizeMake(3,1);
			return [[NSClassFromString(@"CCUIModuleSettings") alloc] initWithPortraitLayoutSize:layoutSize landscapeLayoutSize:layoutSize];
		}
	}
	return %orig;
...
- (MediaControlsPanelViewController *)_createOrReuseViewController {
	MediaControlsPanelViewController *controller = %orig;
	// if (musicModSizeOption == 3) return controller;
	if (controller && [controller isKindOfClass:NSClassFromString(@"MediaControlsPanelViewController")]) {
...
-(void)_adjustForEnvironmentChangeWithSize:(CGSize)size transitionCoordinator:(id)coordinator {
	...
	if (musicModSizeOption != compactDisabledOpt) {
		...
		for (UIViewController *viewController in self.childViewControllers) {
			MediaControlsPanelViewController *controller = (MediaControlsPanelViewController *)viewController;
			if (controller && [controller isKindOfClass:NSClassFromString(@"MediaControlsPanelViewController")]) {
				controller.isBCMMusicModule = YES;
...
-(id)initWithPackFrom:(NSUInteger)packFrom packingOrder:(NSUInteger)packingOrder sizeLimit:(CCUILayoutSize)sizeLimit {
	...
	if (isPad) {
		if (SYSTEM_VERSION_GREATER_THAN_OR_EQUAL_TO(@"12.0")) {
			...
			return %orig(0, 0, CCUILayoutSizeMake(sizeLimit.width, sizeLimit.height));
		} else {
			sizeLimit.width = NSUIntegerMax;
			return %orig(0, 1, CCUILayoutSizeMake(sizeLimit.width, sizeLimit.height));
		}
	} else {
		return %orig;
	}
...
// 	sizeLimit.width = 4;
// 	packFrom = 1;
// 	return %orig(1, 0, CCUILayoutSizeMake(4, sizeLimit.height));
// }
